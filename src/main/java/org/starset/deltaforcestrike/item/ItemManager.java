...
public class ItemManager {
    ...
    private void loadSection(ConfigurationSection section, String path) {
        for (String key : section.getKeys(false)) {
            ...
            // 叶子：有 material，或 armor 套装，或明确 kind
            boolean leaf = child.contains("material")
                    || child.contains("armor")
                    || child.contains("price") && (child.contains("name") || child.contains("type"));

            // 更稳妥：有 name+price 且 (material 或 armor)
            if (child.contains("name") && (child.contains("material") || child.contains("armor"))) {
                try {
                    ...
                    // 短 id 别名：standard -> 若唯一可选用末段
                    String shortId = key;
                    items.putIfAbsent(shortId, item);
                } catch (Exception e) {
    ...
    /** 发放护甲套装到玩家（返回是否成功） */
    public boolean giveArmorSet(org.bukkit.entity.Player player, String id) {
        GameItem def = items.get(id);
    ...
    private ItemStack taggedArmor(Material material, GameItem def) {
        ...
        if (meta != null) {
            ...
            if (plugin.getConfig().getBoolean("player.armor-vanish", true)) {
                Enchantment vanishing = Registry.ENCHANTMENT.get(
    ...
    private Enchantment resolveEnchantment(String key) {
        String k = key.toLowerCase(Locale.ROOT)
                .replace("sharpness", "sharpness")
                .replace("power", "power")
                .replace("unbreaking", "unbreaking");
        // 兼容旧名
        return Registry.ENCHANTMENT.get(org.bukkit.NamespacedKey.minecraft(k));
    }
